             ...
             bool enableDFilter)
{
  ...
  pid->deriv = 0;
  pid->desired = desired;
  pid->prevDesired = desired; // inicializamos PrevDesired con desired?? para calcular la derivada
  pid->prevDeseadoSD = desired;
  pid->kp = kp;
  pid->ki = ki;
  pid->kd = kd;
  pid->kff = kff;
...
float derivada(PidObject *pid)
{
  float outputdev = (pid->desired - pid->prevDesired) / pid->dt;
  pid->prevDesired = pid->desired; // actualizamos prevDesired con desired para la proxima llamada
  return outputdev;
}

float SegDerivada(PidObject *pid)
{
  float outputSegdev = (1.0f/9.8f)*((pid->desired - pid->prevDeseadoSD) / pid->dt); //
  pid->prevDeseadoSD = pid->desired; // actualizamos prevDesired con desired para la proxima llamada
  return outputSegdev;
}
...
void pidReset(PidObject *pid)
{
  ...
  pid->integ = 0;
  pid->deriv = 0;
  pid->prevDesired = pid->desired;
  pid->prevDeseadoSD = pid->desired;
}
