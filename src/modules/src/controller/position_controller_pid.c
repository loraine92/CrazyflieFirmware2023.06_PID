...
static float runDerivada(struct pidAxis_s *axis, float setpoint, float dt) {
  axis->setpoint = setpoint;
  pidSetDesired(&axis->pid, axis->setpoint);
  return derivada(&axis->pid);
}

static float runSegDerivada(struct pidAxis_s *axis, float setpoint, float dt) {
  axis->setpoint = setpoint;
  pidSetDesired(&axis->pid, axis->setpoint);
  return SegDerivada(&axis->pid);
}

float state_body_x, state_body_y, state_body_vx, state_body_vy;
float setpoint_velocidad_x; //cree esta variable para poder hacer el log de setpoint_velocity.x 
float setpoint_velocidad_y; //cree esta variable para poder hacer el log de setpoint_velocity.y 
float input_velocity_x; //cree esta variable para poder hacer el log de input_velocity.x 
float input_velocity_y; //cree esta variable para poder hacer el log de input_velocity.y 
float segunda_derivada_x; //cree esta variable para poder hacer el log de segunda_derivada.x
float segunda_derivada_y; //cree esta variable para poder hacer el log de segunda_derivada.y
float s_attidude_x; //cree esta variable para poder hacer el log de la salida de la suma total, de  attitude->pitch 
float s_attidude_y; //cree esta variable para poder hacer el log de la salida de la suma  total, de  attitude->roll 
float s_pidvelocidad_x; //cree esta variable para poder hacer el log de la salida del PID de velocidad en x
float s_pidvelocidad_y; //cree esta variable para poder hacer el log de la salida del PID de velocidad en y
                                                             ...
                                                             const state_t *state)
{
  ...
  //Estructuras derivadas en X,Y
  Axis3f input_velocity; //array para guardar los datos de la derivada, es lo que le va a ingresar al PID de velocidad X,Y
  Axis3f segunda_derivada;//array para guardar los datos de la segunda derivada en X,Y

  if (setpoint->mode.x == modeAbs) {
    input_velocity.x = runDerivada(&this.pidX, setp_body_x, DT); //Calcula la primera derivada
    segunda_derivada.x = runSegDerivada(&this.pidX, input_velocity.x, DT);
    setpoint_velocity.x = runPid(state_body_x, &this.pidX, setp_body_x, DT); //calcula el pid de posicion
    setpoint_velocidad_x = setpoint_velocity.x; //para hacer el log
    input_velocity_x = input_velocity.x; //para hacer el log
    segunda_derivada_x = segunda_derivada.x; //para hacer el log
  } else if (!setpoint->velocity_body) {
    setpoint_velocity.x = globalvx * cosyaw + globalvy * sinyaw;
  }
  if (setpoint->mode.y == modeAbs) {
    input_velocity.y = runDerivada(&this.pidY, setp_body_y, DT); //Calcula la primera derivada
    segunda_derivada.y = runSegDerivada(&this.pidY, input_velocity.y, DT);
    setpoint_velocity.y = runPid(state_body_y, &this.pidY, setp_body_y, DT);
    setpoint_velocidad_y = setpoint_velocity.y; //para hacer el log
    input_velocity_y = input_velocity.y; //para hacer el log
    segunda_derivada_y = segunda_derivada.y; //para hacer el log
  } else if (!setpoint->velocity_body) {
                                                             ...
                                                             const state_t *state)
{
  ...
  Axis3f s_velocidad;
  // Roll and Pitch
  //attitude->pitch = -runPid(state_body_vx, &this.pidVX, setpoint_velocity->x, DT); Controlador en cascada
  //attitude->roll = -runPid(state_body_vy, &this.pidVY, setpoint_velocity->y, DT); Controlador en cascada
  s_velocidad.x = runPid(state_body_vx, &this.pidVX, input_velocity->x, DT);
  s_velocidad.y = runPid(state_body_vy, &this.pidVY,  input_velocity->y, DT);
  s_pidvelocidad_x = s_velocidad.x; //para hacer el log
  s_pidvelocidad_y =  s_velocidad.y; //para hacer el log
  //attitude->pitch = -(runPid(state_body_vx, &this.pidVX, input_velocity->x, DT) + setpoint_velocity->x + segunda_derivada->x); //controlador en paralelo
  //attitude->roll = -(runPid(state_body_vy, &this.pidVY, input_velocity->y, DT) + setpoint_velocity->y + segunda_derivada->y);  //controlador en paralelo
  attitude->pitch = -(s_pidvelocidad_x + setpoint_velocity->x + segunda_derivada->x);
  attitude->roll = -(s_pidvelocidad_y + setpoint_velocity->y + segunda_derivada->y);
  s_attidude_x = attitude->pitch; //para hacer el log y guardar en la microSD
  s_attidude_y = attitude->roll;  //para hacer el log y guardar en la microSD
...
/**
 * @brief setpoint velocity x, en realidad es la salida del pid de posicion en x
 */
LOG_ADD(LOG_FLOAT, setpVelocidadX, &setpoint_velocidad_x)
/**
 * @brief setpoint velocity y, en realidad es la salida del pid de posicion en y
 */
LOG_ADD(LOG_FLOAT, setpVelocidadY, &setpoint_velocidad_y)
/**
 * @brief derivada del setpoint de x
 */
LOG_ADD(LOG_FLOAT, DevSetpX, &input_velocity_x)
/**
 * @brief derivada del setpoint de y
 */
LOG_ADD(LOG_FLOAT, DevSetpY, &input_velocity_y)
/**
 * @brief segunda derivada del setpoint de x
 */
LOG_ADD(LOG_FLOAT, SegDevSetpX, &segunda_derivada_x)
/**
 * @brief segunda derivada del setpoint de y
 */
LOG_ADD(LOG_FLOAT, SegDevSetpY, &segunda_derivada_y)
/**
 * @brief salida del pid de velocidad en x
 */
LOG_ADD(LOG_FLOAT, s_pidvelocidadX, &s_pidvelocidad_x)
/**
 * @brief salida del pid de velocidad en y
 */
LOG_ADD(LOG_FLOAT, s_pidvelocidadY, &s_pidvelocidad_y)
/**
 * @brief señal de setpoint para el pitch
 */
LOG_ADD(LOG_FLOAT, salida_pitch, &s_attidude_x)
/**
 * @brief señal de setpoint para el roll
 */
LOG_ADD(LOG_FLOAT, salida_roll, &s_attidude_y)
